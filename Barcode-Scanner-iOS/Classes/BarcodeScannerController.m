    ...
    for (AVMetadataObject* metadata in metadataObjects) {
        for (NSString* type in barCodeTypes) {
            if ([metadata.type isEqualToString:type]) {
                ...
                detactedData = [NSDictionary dictionaryWithObjects:@[ detectionString, type ] forKeys:@[ @"value", @"type" ]];
                break;
            }
        }

        if (detectionString != nil) {
            [self performSelector:@selector(detectionFinishWithValue:) withObject:detactedData afterDelay:1.0];
            break;
        }
    }
...
- (void)detectionFinishWithValue:(NSDictionary*)detectedValue
{
    ...
    if (!self.shouldSendReadBarcodeToDelegate) {
        ...
        if ([self.delegate respondsToSelector:@selector(scanViewController:didSuccessfullyScan:)]) {
            [self.delegate scanViewController:self didSuccessfullyScan:detectedValue];
        }
